...
const getTemples = async () => {
    try{
        const response = await fetch("http://byui-cse.github.io/cse121b-ww-course/resources/temples.json");
...
const sortBy = function () {
    ...
    // Use a switch statement with different cases to handle filtering options
    switch (filter) {
        case "utah":
      // Filter for temples in Utah
      const utahTemples = temples.filter(temples => temples.location.toLowerCase().includes("utah"));
      displayTemples(utahTemples);
      break;
    case "nonutah":
      // Filter for temples not in Utah
      const nonUtahTemples = temples.filter(temples => !temples.location.toLowerCase().includes("utah"));
    ...
    case "older":
      ...
      break;
    case "all":
      // Display all temples with no filter
      displayTemples(temples);
      break;
    }
};
